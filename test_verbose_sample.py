...
def run_test(query: str, draft_agent, target_agent, use_fallback: bool):
    ...
    # Verify (simple AST + embedding fallback, no composition mapping)
    if EVAL_AVAILABLE:
...
def main():
    ...
    results_file = os.path.join(results_dir, f"verbose_results_{DATASET}_{timestamp}.json")
